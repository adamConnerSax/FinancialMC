...
getCash::ArrowXml a=>a XmlTree (Maybe A.FMCBaseAsset)    
getCash = proc l -> do
  name    <- getAttrValue "name" -< l
  balance <- readAttrValue "balance" -< l
  returnA -< Just $ A.FMCBaseAsset (AssetCore (T.pack name) balance balance) A.CashAsset
                 
getMixedFund::ArrowXml a=>a XmlTree (Maybe A.FMCBaseAsset)        
getMixedFund = proc l -> do
  ...
  returnA -< Just $ A.FMCBaseAsset (AssetCore (T.pack name) balance basis) (A.MixedFund (pctStock/100) (stkYld/100) (bondInt/100))
                 
getGuaranteedFund::ArrowXml a=>a XmlTree (Maybe A.FMCBaseAsset)        
getGuaranteedFund = proc l -> do
  name    <- getAttrValue "name" -< l
  rate <- readAttrValue "rate" -< l
  balance <- readAttrValue "balance" -< l
  basis <- readAttrValue "paid" -< l
  returnA -< Just $ A.FMCBaseAsset (AssetCore (T.pack name) balance basis) (A.GuaranteedFund (rate/100))


getResidentialRealEstate::ArrowXml a=>a XmlTree (Maybe A.FMCBaseAsset)        
getResidentialRealEstate = proc l -> do    
  name    <- getAttrValue "name" -< l
  value <- readAttrValue "value" -< l
  paid <- readAttrValue "paid" -< l
  returnA -< Just $ A.FMCBaseAsset (AssetCore (T.pack name) value paid) A.ResidentialRE
    
getFixedRateMortgage::ArrowXml a=>a XmlTree (Maybe A.FMCBaseAsset)        
getFixedRateMortgage = proc l -> do    
  ...
  returnA -< Just $ A.FMCBaseAsset (AssetCore (T.pack name) (MV.negate remaining) (MV.negate borrowed)) (A.FixedRateMortgage (rate/100) years)
    
errorAsset::String->FMCXmlArrow XmlTree (Maybe A.FMCBaseAsset)        
errorAsset badTag = addInfoA (Error ("Unrecognized Asset Element \"" ++ badTag ++ "\"")) >>> constA Nothing 

    
getAsset::FMCXmlArrow XmlTree (Maybe A.FMCBaseAsset)  
getAsset = proc l -> do
  tag <- getElemName -< l
  let n = localPart tag
  asset <- case n of
