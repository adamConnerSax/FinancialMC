{-# LANGUAGE DeriveGeneric, ConstraintKinds, FlexibleContexts, TemplateHaskell, TypeFamilies #-}
...
cashEvolveFunction::Evolver CashAsset
...
instance Evolvable CashAsset where
  evolve = cashEvolveFunction
...
mixedFundEvolveFunction::Evolver MixedFund  
mixedFundEvolveFunction mf@(MixedFund _ fracStock stkYield bondInterest) = do
  (flows', newA') <- lift $ do
...
guaranteedFundEvolveFunction::Evolver GuaranteedFund
guaranteedFundEvolveFunction gf@(GuaranteedFund _ rate) = do
...
residentialREEvolveF::Evolver ResidentialRE
...
fixedMortgageEvolveFunction::Evolver FixedRateMortgage
fixedMortgageEvolveFunction frm@(FixedRateMortgage _ rate years) =  do
